package com.myybean.portfolio.domain

import com.myybean.portfolio.domain.constant.SkillType
import com.myybean.portfolio.domain.entity.*
...
class DataInitializer(
    ...
    @PostConstruct // main method가 실행되면 string DI가 component스캔을 하고 의존성 주입을 해서 contruct를 하는데, 그걸 하고 postConstruct 붙은 애들을 찾아서 실행해줌
    // 그 뒤 스프링이 프로젝트가 실행이 됨
    fun initializeDate() {
        println("스프링이 실행되었습니다. 테스트 데이터를 초기화합니다.") // 약어 : sout + 탭. 이거보다 logger를 사용하는 게 더 좋음..!!. 성능에 좋지 않음
        ...
        experience2.addDetails(
            mutableListOf(
                ExperienceDetail(content = "유기묘 위치 공유 서비스 개발", isActive = true), ExperienceDetail(content = "신입 교육 프로그램 우수상 수상", isActive = true)
            ) )
